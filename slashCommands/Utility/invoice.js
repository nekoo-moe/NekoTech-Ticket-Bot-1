...
if(subCmd === "paypal") {
        ...
        if(config.PayPalSettings.Enabled === false) return interaction.reply({ content: "This command has been disabled in the config!", flags: MessageFlags.Ephemeral })
        if(config.PayPalSettings.OnlyInTicketChannels && !ticketDB) return interaction.reply({ content: config.Locale.NotInTicketChannel, flags: MessageFlags.Ephemeral })
        ...
        if(doesUserHaveRole === false) return interaction.reply({ content: config.Locale.NoPermsMessage, flags: MessageFlags.Ephemeral })
        ...
        client.paypal.invoice.create(invoiceObject, async (err, invoice) => {
          if (err) {
            ...
            if(embedSettings.ThumbnailEnabled) {
                ...
                interaction.editReply({ embeds: [embed], components: [row] }).then(async function(msg) {
                  ...
                  await newModel.save();
                    ...
                    let logsChannel = interaction.guild.channels.cache.get(config.PayPalSettings.LogsChannelID);

                const log = new EmbedBuilder()
                .setColor('#FFA726')
                .setAuthor({ 
                    name: config.Locale.PayPalLogTitle, 
                })
                ...
                mainContent += `> **${config.Locale.PayPalPrice}:** \`${config.PayPalSettings.CurrencySymbol}${price}\`\n`;
            ...
            if(config.StripeSettings.Enabled === false) return interaction.reply({ content: "This command has been disabled in the config!", flags: MessageFlags.Ephemeral })
            if(config.StripeSettings.OnlyInTicketChannels && !ticketDB) return interaction.reply({ content: config.Locale.NotInTicketChannel, flags: MessageFlags.Ephemeral })
                ...
                .then(async (invoice) => {
                  ...
                  .addComponents(
                      ...
                      if(embedSettings.ThumbnailEnabled) {
                          ...
                          let logsChannel = interaction.guild.channels.cache.get(config.StripeSettings.LogsChannelID);
          
                          const log = new Discord.EmbedBuilder()
                          .setColor('#FFA726')
                          .setAuthor({ 
                              name: config.Locale.StripeLogTitle, 
                          })
                          .setThumbnail(interaction.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }))
                          .setTimestamp();
                          ...
                          mainContent += `> **${config.Locale.logsExecutor}:** <@!${interaction.user.id}> \`${interaction.user.username}\`\n`;
                          mainContent += `> **${config.Locale.PayPalUser}:** <@!${user.id}> \`${user.username}\`\n`;
                          mainContent += `> **${config.Locale.PayPalPrice}:** \`${config.StripeSettings.CurrencySymbol}${price}\`\n`;
                          mainContent += `> **${config.Locale.PayPalService}:** \`${service}\``;
